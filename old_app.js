    ...
    db.all(`SELECT * FROM USER WHERE user_id = ${user_info.user_id}`, (err, rows) => {
        if (!err) {
            ...
            if (rows == []) {
                ...
                // need to update info if already in there
                db.run(`UPDATE USER                       
                        SET
                        authorization_code = (?),
                        access_token = (?),
                        refresh_token =(?)
                        WHERE user_id = (?)`, [user_info.authorization_code, user_info.access_token, user_info.refresh_token, user_info.user_id], (err) => {
                    if (!err) {
                        console.log("User updated")
                    }
                    else {
                        console.log("ERROR: Updating user in USER")
                    }
                });
